...
public static class MeshGenerator {
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve _heightCurve, int levelOfDetail, bool useFlatShading) {
        ...
        for (int y = 0; y < borderedSize; y += meshSimplificationIncrement) {
            for (int x = 0; x < borderedSize; x += meshSimplificationIncrement) {
                int vertexIndex = vertexIndices[x, y];
                ...
                if (x < borderedSize -1 && y < borderedSize -1) {
                    ...
                    int b = vertexIndices[x + meshSimplificationIncrement, y];
                    int c = vertexIndices[x, y + meshSimplificationIncrement];
                    int d = vertexIndices[x + meshSimplificationIncrement, y + meshSimplificationIncrement];
...
public class MeshData {
    ...
    public MeshData(int verticesPerLine, bool useFlatShading) {
        ...
        borderVertices = new Vector3[verticesPerLine * 4 + 4]; // Since borderSize is vertices * 4 + 4
        borderTriangles = new int[24 * verticesPerLine]; // Since 4 * verticesPerLine (which is 6)
    }
